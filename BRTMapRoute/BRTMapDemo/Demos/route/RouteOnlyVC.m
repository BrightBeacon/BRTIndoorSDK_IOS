...
- (void)viewDidLoad {
    ...
    [BRTDownloader updateMap:kBuildingId AppKey:kAppKey OnCompletion:^(BRTMapUpdate *update, NSError *error) {
        ...
        NSArray<BRTFloorInfo *> *mapInfos = [BRTFloorInfo parseAllMapInfo:kAppKey];
        if (building && mapInfos) {
...
- (void)initRoute:(BRTBuilding *)building info:(NSArray <BRTFloorInfo *> *)mapinfos {
    ...
    startP = [BRTLocalPoint pointWithX:info.mapExtent.xmin Y:info.mapExtent.ymax Floor:info.floorNumber];
    [rm requestRouteWithStart:startP end:[BRTLocalPoint pointWithX:info.mapExtent.xmax Y:info.mapExtent.ymin Floor:info.floorNumber]];
}
