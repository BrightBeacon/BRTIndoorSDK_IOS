...
//  BRTMapProject
//
//  Created by thomasho on 2018/5/18.
//  Copyright © 2018年 thomasho. All rights reserved.
//

#import "RouteOfflineVC.h"
#import <AVFoundation/AVFoundation.h>
#import <BRTMapSDK/BRTOfflineRouteManager.h>

@interface RouteOfflineVC ()<BRTOfflineRouteManagerDelegate>{
...
@property (nonatomic, strong) BRTOfflineRouteManager *offlineRouteManager;

@end

@implementation RouteOfflineVC

- (BRTOfflineRouteManager *)offlineRouteManager {
    if (!_offlineRouteManager) {
        _offlineRouteManager = [BRTOfflineRouteManager routeManagerWithBuilding:self.mapView.building MapInfos:self.mapView.floorInfos];
        _offlineRouteManager.delegate = self;
    }
    return _offlineRouteManager;
}
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)requesRoute {
    if (self.mapView.routeStart&&self.mapView.routeEnd) {
        ...
        self.offlineRouteManager.delegate = self;
        [self.offlineRouteManager requestRouteWithStart:self.mapView.routeStart end:self.mapView.routeEnd stops:self.stops sortStop:NO vehicle:NO ignore:self.fobiddenCid?@[self.fobiddenCid]:nil];
    }
}
...
#pragma mark - **************** 路径规划
- (void)offlineRouteManager:(BRTRouteManager *)routeManager didFailSolveRouteWithError:(NSError *)error
{
...
- (void)offlineRouteManager:(BRTRouteManager *)routeManager didSolveRouteWithResult:(BRTRouteResult *)rs
{
